--1.Retrieve all subjects taught by specific teacher
select * from Subjects

--2.Find the teachers for a particular subjects
...
--3.Get the classes assigned to specific tutor
...
--4.Get a list of all tutors in particular department
select T.Teacher_ID, T.T_Fname, T.T_Lname, D.D_ID, D.D_Name from teachers T 
inner join Departments D  on D.D_ID=T.D_ID where  T.Tutor=1

--5.Count the number of classes for each department
...
--8.Find the most popular subject (taught in the highest number of classes).
select top (1) with ties count(S_ID) from TimeTable  group by S_ID order by count(S_ID) desc

--9.Retrieve information about classes along with the corresponding teacher's name 
...
--10.List all teachers and subjects they teach, including the department information
...
--11.Retrieve all classes for specific department and include the teacher's Information 
...
--12. Retrieve all teachers and classes they are assigned to, including teachers without assigned classes
...
--13.List all rooms and the classes scheduled in each room including rooms without assigned classes
...
--14. Get total number of classes taught by each teacher.
select count(Class_ID) as 'Number of Classes', Teacher_ID from Timetable group by Teacher_ID